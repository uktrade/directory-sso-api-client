...
class UserAPIClient(AbstractAPIClient):
    ...
    def set_user_page_view(self, sso_session_id, service, page):
        ...
        return self.post(
            url, {"service": service, "page": page}, authenticator=authenticator
        )
    ...
    def get_user_lesson_completed(self, sso_session_id, service, lesson_page):
        ...
        return self.get(
            url,
            {"service": service, "lesson": lesson_page},
            authenticator=authenticator,
        )

    def set_user_lesson_completed(
        self, sso_session_id, service, lesson_page, lesson, module
    ):
        authenticator = AuthenticatorNegotiator(sso_session_id=sso_session_id)
        url = self.endpoints["user_lesson_completed"]
        return self.post(
    ...
    def delete_user_lesson_completed(self, sso_session_id, service, lesson):
        ...
        url = self.endpoints["user_lesson_completed"]
        return self.delete(
            url, {"service": service, "lesson": lesson}, authenticator=authenticator
        )
    ...
    def set_user_questionnaire_answer(
        self, sso_session_id, service, question_id, answer
    ):
        url = self.endpoints["user_questionnaire"]
        return self.post(
            url,
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, max=10))
    def user_logout(self, cookies=None):
        if cookies is None:
            cookies = {}
