...
class ValidateSecurityScanLocalTest(TestCase):
    ...
    def test_main_returns_error_when_commit_msg_file_missing(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "sys.argv", ["validate_security_scan_local.py", str(missing_path)]
            ):
                result = main()
    ...
    def test_main_appends_signed_off_by_trailer(self):
        with TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "sys.argv", ["validate_security_scan_local.py", str(commit_msg_path)]
            ):
                result = main()

            assert result == 0
            assert (
                commit_msg_path.read_text(encoding="utf-8")
